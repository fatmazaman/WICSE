...
(function(c) {
    ...
    c.tools.overlay = {
        ...
        conf: {
            ...
            closeSpeed: "fast",
            effect: "default",
            fixed: !document.all || window.XMLHttpRequest,
            left: "center",
            autoFit: true,
            load: false,
            mask: null,
    ...
    var d = [],
        b = {};
    c.tools.overlay.addEffect("default", function(h, g) {
        var f = this.getConf(),
            e = c(window);
    ...
    function a(h, m) {
        var p = this,
            f = h.add(p),
            o = c(window),
            k, j, i, e = c.tools.expose && (m.mask || m.expose),
        ...
        function n(s, r, q) {
            ...
            s[1] = parseFloat(s[1]) / q;
            s[2] = parseFloat(s[2]) / r;
            s[3] = parseFloat(s[3]) / q;
            return s.join("px ") + "px"
        }
        c.extend(p, {
            load: function(r) {
                ...
                q[0].call(p, this.getPos(), function() {
                    if (i) {
                        r.type = "onLoad";
                        f.trigger(r)
                    }
                ...
                if (e && m.closeOnClick) {
                    c.mask.getMask().one("click", p.close)
                }
                if (m.closeOnClick) {
                    c(document).on("click." + l, function(s) {
                        if (!c(s.target).parents(j).length) {
                            p.close(s)
                        }
                    })
                }
                if (m.closeOnEsc) {
                    c(document).on("keydown." + l, function(s) {
                        if (s.keyCode == 27) {
                            p.close(s)
                        }
                    })
                ...
                if (m.autoFit) {
                    c(window).on("resize scroll", p.putOverlay)
                }
                return p
            },
            close: function(q) {
                ...
                c(window).off("resize scroll", p.putOverlay);
                if (e) {
                    c.mask.close()
                }
                return p
            ...
            getPos: function() {
                ...
                if (!s) {
                    j.data("oSize", s = {
                        ...
                        kx: q / j.width(),
                        ky: r / j.height(),
                        padding: [j.css("padding-top"), j.css("padding-right"), j.css("padding-bottom"), j.css("padding-left")].join(" ")
                    })
                }
                if (m.autoFit) {
                    if (s.width > o.width()) {
                        q = o.width();
                        r = s.height * o.width() / s.width
                    } else {
                        q = s.width;
                        r = s.height
                    }
                }
                if (typeof t.top == "string") {
                    t.top = t.top == "center" ? Math.max((o.height() - r) / 2, 0) : parseInt(t.top, 10) / 100 * o.height()
                }
                if (t.left == "center") {
                    t.left = Math.max((o.width() - q) / 2, 0)
                }
                if (!m.fixed) {
                    t.top += o.scrollTop();
                    t.left += o.scrollLeft()
                ...
                if (m.autoFit) {
                    if (r < o.height()) {
                        t.top = Math.min(t.top, o.height() - r + o.scrollTop())
                    } else {
                        t.top = parseInt(c(j).css("top")) || 0;
                        t.top = Math.max(t.top, o.height() - r + o.scrollTop());
                        t.top = Math.min(t.top, o.scrollTop())
                    }
                    t.width = q / s.kx;
                    t.height = r / s.ky;
                    t.oWidth = q;
                    t.oHeight = r;
                    t.padding = n(s.padding, s.width / q, s.height / r)
                }
                t.position = m.fixed ? "fixed" : "absolute";
                return t
            },
            putOverlay: function() {
                c(j).css(p.getPos())
        ...
        c.each("onBeforeLoad,onStart,onLoad,onBeforeClose,onClose".split(","), function(r, q) {
            if (c.isFunction(m[q])) {
                c(p).on(q, m[q])
            }
            p[q] = function(s) {
...
(function(e) {
    ...
    function d(g) {
        var h = g.offset();
        return {
            top: h.top + g.height() / 2,
            left: h.left + g.width() / 2
        }
    }
    var f = function(s, r) {
        var m = this.getOverlay(),
            q = this.getConf(),
            j = this.getTrigger(),
            t = this,
            u = s.oWidth,
            o = m.data("img");
        var n = m.css("backgroundImage");
        if (!o) {
            if (!n) {
        ...
        var l = q.start.top || Math.round(a.height() / 2),
            i = q.start.left || Math.round(a.width() / 2);
        if (j) {
            var h = d(j);
            l = h.top;
            i = h.left
        ...
        function k() {
            m.css("zIndex", q.zIndex + 1).fadeIn(q.fadeInSpeed, function() {
                if (t.isOpened() && !e(this).index(m)) {
                    r.call()
                } else {
                    m.hide()
                }
        ...
        if (o) {
            o.animate({
                ...
                left: s.left,
                width: u
            }, q.speed, k).css("position", s.position)
        } else {
            k()
        } if (o) {
            var g = this.putOverlay;
            this.putOverlay = function() {
                g();
                var p = t.getPos();
                o.css({
                    top: p.top,
